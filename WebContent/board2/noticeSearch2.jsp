...
// 제네릭타입으로 데이터 가져오면  형변환없이 사용가능
       ...
       %>
<tr onclick="location.href='fcontent2.jsp?num=<%=bb.getNum()%>'">
      <td><%=bb.getNum() %></td>
...
<div id="table_search">
<form action="noticeSearch2.jsp" method="get">
...
if(startPage > pageBlock){
   %><a href="noticeSearch2.jsp?pageNum=<%=startPage-pageBlock%>&search=<%=search%>">Prev</a><%
}
for(int i=startPage;i<=endPage;i++){
   %><a href="noticeSearch2.jsp?pageNum=<%=i%>&search=<%=search%>"><%=i %></a><%
}
if(endPage < pageCount){
   %><a href="noticeSearch2.jsp?pageNum=<%=startPage+pageBlock%>&search=<%=search%>">Next</a><%
}
%>
</div>
</article>
